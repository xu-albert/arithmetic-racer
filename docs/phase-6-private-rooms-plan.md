...
| Race parameters | Creator sets difficulty + length in lobby before Start; defaults `medium` / 20 | Per requirement |
...
- `onMessage(connection, raw)`: parse JSON; ignore non-objects; dispatch on `type`:
  ...
  - `set-config`: require `isCreator` AND `state === 'lobby'`. Validate difficulty ∈ DIFFICULTIES, raceLength ∈ [5, 50] integer. Update. Broadcast `config-changed` + `state`. On unauthorized: send `error` (`NOT_CREATOR` or `BAD_STATE`).
