...
| `worker/*.test.js` | `vitest` | Worker-level helpers with D1/binding dependencies: `auth.test.js` (better-auth account linking and password reset, driven through the real handler), `email.test.js`, `log-throttle.test.js`, `logger.test.js`, `plausibility.test.js`, `race-result-store.test.js`, `race-score.test.js`, `rate-limit.test.js`, `user-agent.test.js`, `username-validator.test.js`, `version.test.js` |
...
| 23 | **Account takeover through Google sign-in**: better-auth 1.6.9 matched a Google sign-in to an existing user by email and linked it, marking the email verified, without asking whether the local row's email was ever verified. Nothing verifies email at sign-up, so a stranger could register a password account under someone else's address, wait for the owner to sign in with Google, and keep a password into the owner's account. An `account.create.before` hook in `worker/auth.js` now refuses to attach an OAuth identity to an existing unverified user; new Google users, returning ones and verified users pass. better-auth 1.7 closes this upstream (`accountLinking.requireLocalEmailVerified`). A review round kept the refusal from stranding the address's real owner: better-auth reports it as a generic `unable_to_link_account` on its own error page, so the client now passes `errorCallbackURL` and an after hook names the reason (`ACCOUNT_LINK_REQUIRES_VERIFIED_EMAIL`), which reopens the Log In modal with an explanation; and a completed password reset — the one proof of inbox control the app has — marks the email verified and revokes every session, so the owner can take a squatted address back and the squatter's session ends with it. | `fix(auth): refuse to link Google sign-in into an unverified account` + the `no-mistakes(review)` fix | `worker/auth.test.js` ("Google sign-in into a pre-registered unverified account", "the owner taking a squatted address back with a password reset", "Google sign-in that must keep working"), `public/src/auth.test.js` | GUARDED |
...
- **Auth**: better-auth handles email/password and Google OAuth (`worker/auth.js`,
  `worker/session.js`). `worker/auth.test.js` covers Google sign-in, account linking and
  password reset (§4 #23); the rest of the auth flow is exercised only incidentally via
