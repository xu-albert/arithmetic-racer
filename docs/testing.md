...
| --- | --- | --- |
| `public/src/*.test.js` | `node --test` | Client logic: `runner.js`/`remote-runner.js` (race loop), `game.js` (problem generation, difficulty), `bot.js`/`bot-timeline.js`, `handles.js`, `header.js`, `profile.js`, `room-config-rules.js`, `room-expiry.js`, `ui.js` (race screen, over a DOM stub), `lobby.js` (the race-handoff gate, over stubbed PartySocket + DOM), `seeded-rng.js`, `username-validator-client.js`, `recent-finishes.js`, `leaderboard.js`, `leaderboard-period.js`, `bug-report-context.js`, `auto-start.js`, `captcha-session.js`, `solo-result.js` (what a solo race reports — nothing for a quit), `room-client.js` (`hello` first on every connection, then sends paced under the room's per-socket limit, judged by the real `server/socket-limit.js`) |
...
| 22 | **Race time on the device's clock, and reconnect leaks**: the remote runner read bot timelines and the provisional finish as `Date.now()` minus the room's `raceStartedAt`, so a device clock a minute fast finished every Quick Match bot at once. It now reads race time through an offset estimated from the `serverNow` the room stamps on `race-start`, `bot-timelines` and every `state`. Same PR also fixed: answers typed offline reaching the room ahead of the `hello` that re-seats the socket, and past its 20-per-second socket limit (`room-client.js` now holds them itself and sends `hello` first, then the rest at half that budget); a `finished` snapshot podiuming the local bot replay instead of the room's rows (now on `lastRace.botRows`); `runner.sequence` staying on the sequence the runner was built with; and quit solo races being stored (the client posts nothing, and `POST /api/race-result` refuses `finished: false` with 422; the profile's finish rate now counts multiplayer races only, since solo rows are all finishes). | b0f9633 (`fix(race): read bot pace on the room's clock, keep reconnects honest, and stop storing solo quits`) | `public/src/remote-runner.test.js` ("race time is read on the room's clock", "the exposed sequence"), `public/src/room-client.test.js`, `server/public-room.test.js` (the `serverNow` stamps, `lastRace.botRows`), `public/src/solo-result.test.js`, `worker/routes/race-result.test.js` ("unfinished races are not stored"), `worker/routes/me.test.js` + `public/src/profile.test.js` (the multiplayer finish rate) | GUARDED |
