...
| --- | --- | --- |
| `public/src/*.test.js` | `node --test` | Client logic: `runner.js`/`remote-runner.js` (race loop), `game.js` (problem generation, difficulty), `bot.js`/`bot-timeline.js`, `handles.js`, `header.js`, `profile.js`, `room-config-rules.js`, `room-expiry.js`, `ui.js` (race screen, over a DOM stub), `lobby.js` (the race-handoff gate, over stubbed PartySocket + DOM), `seeded-rng.js`, `username-validator-client.js`, `recent-finishes.js`, `leaderboard.js`, `leaderboard-period.js`, `bug-report-context.js`, `auto-start.js`, `captcha-session.js`, `auth.js` (the Google sign-in request and the return page's error handling), `solo-result.js` (what a solo race reports — nothing for a quit), and the one local patch to the vendored partysocket (`partysocket-flush-order.test.js`) |
| `server/*.test.js` (vitest) | `vitest` | Room DO behavior: `room-identity.test.js`, `public-room.test.js`, `room-config.test.js`, `room-handles.test.js`, `room-winddown.test.js`, `room-allocation.test.js`, `room-captcha.test.js`, `room-captcha-e2e.test.js`, `room-race-deadline.test.js`, `room-answer-persistence.test.js`, `room-result-outbox.test.js`, `private-room-ten-players.test.js`, `lobby-router.test.js`, `socket-limit.test.js` |
...
| `worker/*.test.js` | `vitest` | Worker-level helpers with D1/binding dependencies: `auth.test.js` (better-auth Google sign-up and account linking, and what a password reset verifies and detaches, driven through the real handler), `claim.test.js` (`runClaim` in `auth.js`), `email.test.js`, `log-throttle.test.js`, `logger.test.js`, `plausibility.test.js`, `race-result-store.test.js`, `race-score.test.js`, `rate-limit.test.js`, `user-agent.test.js`, `username-validator.test.js`, `version.test.js` |
...
| 11 | Disconnect-timeout path did not write a DNF (`finished=0`) row distinct from an explicit quit. | 2448b8c (`disconnect-timeout writes DNF row; split manual matrix R3`) | `server/room-race-deadline.test.js` ("a racer who had NOT finished is still dropped when they leave" drives `removePlayer` past the 30s reconnect grace and asserts the `finished = 0` row, alongside the `handleQuit` path); manual R3b in §6 | GUARDED |
| 12 | Race-end logic ended the race before the slower player finished; animation stutter; missing `(Guest)` badge. | 7694dca | `public/src/recent-finishes.test.js` covers the Guest-badge convention broadly; `server/room-race-deadline.test.js` ("a racer who finishes inside the grace counts normally") isolates the race-end-waits-for-slowest-player invariant in the bounded form it now has — the wait ends at the race deadline (#18), not whenever the slower player gets there | PARTIALLY GUARDED |
...
| 20 | **Wrong answers were not durable**: `handleAnswer`'s wrong-answer branch bumped `attempts` and reset `currentStreak` without persisting. A private room happened to survive it because `onMessage()` ends in `flushActivity()`, but that is gated on `expiresWhenIdle()` and so is a no-op in a public room — a Quick Match room hibernated or evicted before the next correct answer woke with neither the attempt nor the broken streak, overstating the stored accuracy and the longest streak. The branch now calls `persist()` itself; `flushActivity()` is the idle clock's write, never a handler's. | c16f3bc (`fix(room): persist wrong-answer stats before hibernation`) | `server/room-answer-persistence.test.js` (reads the row back out of `ctx.storage` rather than inspecting `room.state`, for both room classes, and pins the write count so a correct answer still persists exactly once) | GUARDED |
| 21 | **Private room-name collision**: `POST /api/rooms` sampled a name from the 13,248-name list and returned it whether or not a live room already owned it, so the caller was handed somebody else's lobby as their brand-new private room — in it, but not its creator. One draw hits a live room with probability live/13248, and the chance that some pair among 50 allocated names collides is already ~8.8% (~31.2% at 100). Creation now *reserves*: `allocateRoomId()` retries up to `ROOM_ID_ATTEMPTS` draws against `reserveRoomName()`, whose read-then-write inside one Durable Object RPC is what makes a claim atomic, and an exhausted budget is a `503` rather than a fallback to a live name. Reserving is what turned a live row into the thing that makes a name unreservable, so every path that mints live state with nobody in the room — `reserveRoomName()` and `onStart()`, the latter reached by a bare `GET /parties/race-room/<name>` since partyserver initializes before it checks for an upgrade — marks it `unjoined` and winds it down on `UNJOINED_ROOM_IDLE_MS` (2 min) instead of the 30-minute idle clock; otherwise a few unauthenticated requests a second would hold the whole namespace, the GET path permanently. Full invariant writeup: `AGENTS.md` "A room name is reserved, never merely drawn". | ba3eb1f (`fix(room): reserve a room name instead of sampling one`) + b1ae236 / 2d874b4 (`no-mistakes(review)` fixes: the unjoined fuse) + a12016f (`no-mistakes(test)` fix: persisting the room name so the fuse's alarm can expire it) | `server/room-allocation.test.js` | GUARDED |
| 22 | **Race time on the device's clock, and reconnect leaks**: the remote runner read bot timelines and the provisional finish as `Date.now()` minus the room's `raceStartedAt`, so a device clock a minute fast finished every Quick Match bot at once. It now reads race time through an offset estimated from the `serverNow` the room stamps on `race-start`, `bot-timelines` and every `state`. Same PR also fixed: the vendored partysocket flushing answers typed offline ahead of the `hello` that re-seats the socket (patched in place — `npm run vendor` would revert it; the flush is still one burst, so a queue past the room's 20-per-second socket budget loses the excess, a known follow-up); a `finished` snapshot podiuming the local bot replay instead of the room's rows (now on `lastRace.botRows`); `runner.sequence` staying on the sequence the runner was built with; and quit solo races being stored (the client posts nothing, and `POST /api/race-result` refuses `finished: false` with 422; the profile's finish rate now counts multiplayer races only, since solo rows are all finishes). | b0f9633 (`fix(race): read bot pace on the room's clock, keep reconnects honest, and stop storing solo quits`) | `public/src/remote-runner.test.js` ("race time is read on the room's clock", "the exposed sequence"), `public/src/partysocket-flush-order.test.js`, `server/public-room.test.js` (the `serverNow` stamps, `lastRace.botRows`), `public/src/solo-result.test.js`, `worker/routes/race-result.test.js` ("unfinished races are not stored"), `worker/routes/me.test.js` + `public/src/profile.test.js` (the multiplayer finish rate) | GUARDED |
| 23 | **Race-result writes were fire-and-forget**: the room only logged a failed insert, so a D1 outage at race end lost the row for good (nothing rewrites a finished race), and a crash that replayed a persisted mid-race snapshot re-ran the race end and inserted every row twice. Rows now go through a durable outbox (`state.pendingResults`), persisted before the first insert attempt and retried from the room's alarm for up to `RESULT_OUTBOX_TTL_MS` — held across idle cleanup, the 24h ceiling, the expiry tombstone and a name re-reservation — and the insert skips a row whose (room, device, race time, result) fingerprint already landed. A room row is dated to its race, not to the write, so a retry past UTC midnight stays in its leaderboard window. Mechanism: `queueRaceResults` / `drainRaceResults` in `server/room.js`, `insertRaceResult` in `worker/race-result-store.js`. | 5e3876b (`fix(server): make race-result writes durable and idempotent`) + 8897826 / 89ed37c / 6b44956 / e932e02 (`no-mistakes(review)` fixes) | `server/room-result-outbox.test.js` (both room classes, against a D1 binding that throws), `worker/race-result-store.test.js` ("room rows") | GUARDED |
| 24 | **Anonymous-history claim took a device's whole past, unrecorded**: signing up (the email/password hook, or the first username set after Google sign-in) moved *every* anonymous `race_results` row on the presented `deviceId` to the new account and left no trace. That id sits in `localStorage` and is the claim's only proof, so whoever held it could take all of a device's history. There is no anonymous-side credential to check instead, so the claim is made smaller rather than safe: `runClaim` (`worker/auth.js`) only moves rows whose server-stamped `played_at` is inside `CLAIM_WINDOW_MS` (7 days), and writes a `history_claims` row (migration `0010`) in the same D1 batch as the UPDATE, so no claim commits without its record. The admin dashboard lists those records. A `deviceId` longer than `MAX_DEVICE_ID_LENGTH` can match no stored race, so it runs neither write. | 6a50fc9 (`fix(worker): limit the anonymous-history claim to the last week and log every claim`) + c6044e5 / e97b14e (`no-mistakes(review)` fixes) | `worker/claim.test.js`, `worker/routes/admin.test.js` ("admin dashboard — history claims"), `migrations/migrations.test.js` ("0010 — history_claims") | GUARDED |
| 25 | **Account takeover through Google sign-in**: better-auth 1.6.9 matched a Google sign-in to an existing user by email and linked it, marking the email verified, without asking whether the local row's email was ever verified. Nothing verifies email at sign-up, so a stranger could register a password account under someone else's address, wait for the owner to sign in with Google, and keep a password into the owner's account. An `account.create.before` hook in `worker/auth.js` now refuses to attach an OAuth identity to an existing unverified user; new Google users whose email Google verified, returning ones and verified users pass. better-auth 1.7 closes this upstream (`accountLinking.requireLocalEmailVerified`). A review round kept the refusal from stranding the address's real owner: better-auth reports it as a generic `unable_to_link_account` on its own error page, so the client now passes `errorCallbackURL` and an after hook names the reason (`ACCOUNT_LINK_REQUIRES_VERIFIED_EMAIL`), which reopens the Log In modal with an explanation; and a completed password reset — the one proof of inbox control the app has — marks the email verified and revokes every session, so the owner can take a squatted address back and the squatter's session ends with it. A second review round closed the same squat held through Google instead of a password: a `user.create.before` hook refuses a Google sign-up whose email Google reports unverified before the user row is written (reason `OAUTH_EMAIL_NOT_VERIFIED`, explained in the app the same way), and a reset of a still-unverified user also detaches every OAuth identity attached to it, so a stranger's Google sign-in stops reaching the account and the owner can link their own afterwards. | `fix(auth): refuse to link Google sign-in into an unverified account` + the `no-mistakes(review)` fixes | `worker/auth.test.js` ("Google sign-in into a pre-registered unverified account", "the owner taking a squatted address back with a password reset", "Google sign-up with an email Google has not verified", "Google sign-in that must keep working"), `public/src/auth.test.js` | GUARDED |
...
| R3b | Two players racing; one closes their tab and waits past the 30s reconnect grace | Two rows; the disconnected player has `finished = 0`, `finish_time_ms = NULL` (covers the `removePlayer` path, distinct from R3a's `handleQuit` path — regression #11 in §4, now also guarded by `server/room-race-deadline.test.js`) |
...
| R6 | Solo Quickplay race (regression check) | Finished: one row written via the route; `room_id = NULL`; existing solo stats behavior unchanged. Quit: no row, and no request is sent |
| R7 | Any finished race (solo or room) | Row has non-NULL `points`; a quit room race (R3a/R3b) has `points NULL` |
...
- **Auth**: better-auth handles email/password and Google OAuth (`worker/auth.js`,
  `worker/session.js`). `worker/auth.test.js` covers Google sign-in (an unverified Google
  email cannot create a user), account linking (never into an unverified user) and password
  reset (verifies the email, detaches OAuth identities an unverified user carried, ends every
  session) (§4 #25); the rest of the auth flow is exercised only incidentally via
  `worker/routes/me.test.js`, and the signup claim hook that `worker/claim.test.js` drives
  through the real sign-up endpoint (§4 #24) — flagged as a gap (§11).
...
| Private room-name allocation (no collision handed back) | Automated (part of `npm test`) | `server/room-allocation.test.js` — forced collisions, an exhausted budget, the ordinary single-draw path, and the short hold on any room nobody joins (reservation or bare GET) |
...
| P2 | Auth-flow coverage stops at Google sign-in, account linking and password reset (`worker/auth.test.js`); email/password sign-up validation and session expiry have only incidental route coverage. | An auth regression in those paths could ship without any test failing. | M — extend `worker/auth.test.js` over those paths. |
