...
| `worker/*.test.js` | `vitest` | Worker-level helpers with D1/binding dependencies: `claim.test.js` (`runClaim` in `auth.js`), `email.test.js`, `log-throttle.test.js`, `logger.test.js`, `plausibility.test.js`, `race-result-store.test.js`, `race-score.test.js`, `rate-limit.test.js`, `user-agent.test.js`, `username-validator.test.js`, `version.test.js` |
...
| 25 | **Hostless private room**: succession ran only when a seat was spliced, so a host whose socket closed mid-race and whose reconnect grace expired kept `isCreator` on the departed seat the room holds for its result row — the finished room had no live host, and Race Again was `NOT_CREATOR` for everyone until idle winddown. The flag now follows presence on every transition (`ensureLiveCreator()` in `server/room.js`, whose doc comment owns the rule). Same PR also: a countdown whose last human leaves cancels back to the lobby in both room types instead of running a ghost or bots-only race; a reissued captcha settles the previous one as `captcha_superseded` rather than `captcha_timeout`; and Quick Match bot timelines land in the same write as the racing snapshot, with an alarm-time heal for a room a lost second write left without them. | be4a9f2 (`fix(server): succeed departed room hosts, cancel empty countdowns, honest captcha supersede, atomic bot timelines`) + cb37930 / f7189a4 (`no-mistakes(review)` fixes) | `server/room-lifecycle.test.js`, `server/public-room.test.js` ("last human leaving a countdown cancels it…", "bot timelines — one write, self-healing"), `server/room-captcha.test.js` ("a superseded challenge"), `public/src/captcha-session.test.js` | GUARDED |
...
- **Auth**: better-auth handles email/password and Google OAuth (`worker/auth.js`,
  ...
  incidentally exercise via `worker/routes/me.test.js`, and the signup claim hook that
  `worker/claim.test.js` drives through the real sign-up endpoint (§4 #24) — flagged as a
  gap (§11).
