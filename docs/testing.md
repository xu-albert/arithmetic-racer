...
| 25 | **Offline answers lost on reconnect**: answers typed while the socket was down sat in partysocket's queue and went out after `hello` as one unpaced burst, so an outage longer than the room's 20-per-second socket budget lost the excess (row 22's follow-up). The remote runner now holds them in a per-race outbox of `{index, value}` and sends one `catch-up` message after `hello`. The room grades it by position — below the seat's score skipped, equal graded exactly as `handleAnswer` does, above reported as a gap — once per `batchId`, rejects a batch over `CATCHUP_MAX_ENTRIES_PER_PROBLEM` × race length (`public/src/catch-up-rules.js`, shared by both halves) whole, and answers with one persist, one coalesced `advance` and a targeted `catch-up-ack`. The ack goes even to a socket with no seat, because the client pauses input behind a "Catching up…" pill until it arrives. A catch-up finish arms the race grace and the captcha like a live one. Mechanism: `handleCatchUp` / `gradeCatchUp` in `server/room.js`; the half-open-socket limit is noted at the outbox in `public/src/remote-runner.js`. | 25e02ff (`feat(rooms): grade offline answers as one positional catch-up batch`) + 8392960 / d72b7e9 / c33570f (`no-mistakes(review)` fixes) | `server/room-catchup.test.js`, `public/src/catch-up.test.js`, `public/src/ui.test.js` ("the catch-up drain pause") | GUARDED |
...
| 14c | Take one tab offline mid-race, type several answers (a wrong one among them), then come back online before the race ends | While offline the input keeps working and the car moves on each correct answer. On reconnect a **Catching up…** pill briefly shows and the input is disabled until the room has graded the batch; the tab then resumes from the room's position with every offline answer counted, and the other browser sees that car jump forward once (regression #25 in §4). |
