...
| `worker/*.test.js` | `vitest` | Worker-level helpers with D1/binding dependencies: `auth.test.js` (better-auth Google sign-up and account linking, and what a password reset verifies and detaches, driven through the real handler), `email.test.js`, `log-throttle.test.js`, `logger.test.js`, `plausibility.test.js`, `race-result-store.test.js`, `race-score.test.js`, `rate-limit.test.js`, `user-agent.test.js`, `username-validator.test.js`, `version.test.js` |
...
- **Auth**: better-auth handles email/password and Google OAuth (`worker/auth.js`,
  ...
  session) (§4 #23); the rest of the auth flow is exercised only incidentally via
  `worker/routes/me.test.js` — flagged as a gap (§11).
