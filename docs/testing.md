...
| `worker/*.test.js` | `vitest` | Worker-level helpers with D1/binding dependencies: `auth.test.js` (better-auth account linking, driven through the real handler), `email.test.js`, `log-throttle.test.js`, `logger.test.js`, `plausibility.test.js`, `race-result-store.test.js`, `race-score.test.js`, `rate-limit.test.js`, `user-agent.test.js`, `username-validator.test.js`, `version.test.js` |
...
- **Third-party services**: none are exercised in tests, because none are meaningfully
  ...
  better-auth's Google OAuth flow is driven end to end by `worker/auth.test.js`, with only
...
| 23 | **Account takeover through Google sign-in**: better-auth 1.6.9 matched a Google sign-in to an existing user by email and linked it, marking the email verified, without asking whether the local row's email was ever verified. Nothing verifies email at sign-up, so a stranger could register a password account under someone else's address, wait for the owner to sign in with Google, and keep a password into the owner's account. An `account.create.before` hook in `worker/auth.js` now refuses to attach an OAuth identity to an existing unverified user; new Google users, returning ones, verified users and explicit `/link-social` linking pass. better-auth 1.7 closes this upstream (`accountLinking.requireLocalEmailVerified`). | `fix(auth): refuse to link Google sign-in into an unverified account` | `worker/auth.test.js` ("Google sign-in into a pre-registered unverified account", "Google sign-in that must keep working") | GUARDED |
...
- **Auth**: better-auth handles email/password and Google OAuth (`worker/auth.js`,
  `worker/session.js`). `worker/auth.test.js` covers Google sign-in and account linking
  (§4 #23); the rest of the auth flow is exercised only incidentally via
...
| P2 | Auth-flow coverage stops at Google sign-in and account linking (`worker/auth.test.js`); email/password sign-up validation, password reset and session expiry have only incidental route coverage. | An auth regression outside linking could ship without any test failing. | M — extend `worker/auth.test.js` over those paths. |
